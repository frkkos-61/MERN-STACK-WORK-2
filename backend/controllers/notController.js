...
//! Tarihe göre sıralama sort ile yapılır -1 koy en son eklenen en başa gelir
...
const notGetir = async (req, res) => {
  ...
  //! id'mizin veri tabanındaki tipine uyumlu olup olmadığını kontrol etmeliyiz
...
const notGuncelle = async (req, res) => {
  ...
  const not = await NotModel.findOneAndUpdate({ _id: id }, { ...req.body },{new:true});
